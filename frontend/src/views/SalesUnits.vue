<template>
  <el-card>
    <div style="margin-bottom:12px; display:flex; justify-content: space-between; align-items:center;">
      <div>
        <el-button type="primary" @click="openCreate">新增销售单位</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑销售单位' : '新增销售单位'" width="520px">
...
const bulkJson = ref('[\n  {"name":"销售单位A","contact":"张三","phone":"123456"}\n]');
const formRef = ref();
...
const submitBulk = async () => {
  ...
  try {
    loading.value = true;
    await axios.post('/api/sales-units/bulk', body);
...
const bulkDelete = async () => {
  ...
  try {
    loading.value = true;
    await axios.post('/api/sales-units/bulk', { deleteIds: multipleSelection.value.map(i => i.id) });
...
const submit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      if (form.id) {
        await axios.put(`/api/sales-units/${form.id}`, form);
        ElMessage.success('更新成功');
      } else {
        await axios.post('/api/sales-units', form);
...
const remove = async (row) => {
  try {
    await ElMessageBox.confirm(`确认删除销售单位：${row.name}?`, '提示', { type: 'warning' });
  } catch (e) {
    return;
  }
  deletingId.value = row.id;
  try {
    await axios.delete(`/api/sales-units/${row.id}`);
